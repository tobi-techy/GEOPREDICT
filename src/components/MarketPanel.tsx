...
export default function MarketPanel({ market, onClose, myYesStake = 0, myNoStake = 0, onBetPlaced }: Props) {
  ...
  const handleBet = () => {
    ...
    if (!val || val <= 0) return;
    // TODO: integrate Cofhe SDK encrypt + contract write
    onBetPlaced?.(activeTab, val);
    setStakeInput('');
  };
      ...
      <div className="fixed right-0 top-0 z-40 w-full sm:w-[78%] h-full bg-[#0b0d10] border-l border-white/[0.06] flex flex-col overflow-hidden">
        ...
        <div className="flex items-start gap-3 px-5 pt-3 pb-2 border-b border-white/[0.06]">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1.5">
              <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded bg-violet-500/15 border border-violet-500/25 text-[10px] font-bold text-violet-300 uppercase tracking-wider">
                <span className="w-1.5 h-1.5 rounded-full bg-violet-400 animate-pulse" />
                {src}
              </span>
              <span className="px-2 py-0.5 rounded bg-emerald-500/10 border border-emerald-500/20 text-[10px] font-bold text-emerald-300 uppercase tracking-wider">
                🔒 FHE Encrypted
              </span>
            </div>
        ...
        <div className="grid grid-cols-5 border-b border-white/[0.06] text-center">
          {[
            ...
            { l: 'Closes', v: market.deadline.toLocaleDateString('en-US', { month: 'short', day: 'numeric' }), c: 'text-white/80' },
          ].map(s => (
          ...
          <div className="w-[280px] shrink-0 border-l border-white/[0.06] flex flex-col bg-[#0e1014] overflow-y-auto">
            {isResolved ? (
              <div className="flex-1 flex flex-col p-5 gap-4">
                ...
                <p className="text-[11px] text-white/30 text-center">Claim flow coming in Wave 2</p>
              </div>
            ) : (
              <div className="flex flex-col h-full">
                {/* Yes / No tabs */}
                <div className="grid grid-cols-2">
                  <button onClick={() => setActiveTab(1)} className={`py-3 text-[14px] font-bold transition-all ${activeTab === 1 ? 'text-emerald-400 bg-emerald-500/10 border-b-2 border-emerald-400' : 'text-white/35 hover:text-white/55 border-b border-white/[0.06]'}`}>YES</button>
                  <button onClick={() => setActiveTab(2)} className={`py-3 text-[14px] font-bold transition-all ${activeTab === 2 ? 'text-rose-400 bg-rose-500/10 border-b-2 border-rose-400' : 'text-white/35 hover:text-white/55 border-b border-white/[0.06]'}`}>NO</button>
                </div>
                ...
                <div className="flex-1 p-5 flex flex-col gap-4">
                  <div>
                    ...
                    <div className="flex items-center gap-2 px-4 py-3 rounded-xl bg-white/[0.04] border border-white/[0.08] focus-within:border-white/[0.2] transition-colors">
                      <span className="text-white/30 text-lg">Ξ</span>
                      <input
                        type="number"
                        value={stakeInput}
                      ...
                      />
                      <span className="text-[12px] text-white/25 font-medium">ETH</span>
                    </div>
                  </div>

                  <div className="grid grid-cols-5 gap-1.5">
                    {['0.001', '0.005', '0.01', '0.05', '0.1'].map(v => (
                      <button key={v} onClick={() => setStakeInput(v)} className="py-2 rounded-lg bg-white/[0.04] hover:bg-white/[0.08] border border-white/[0.06] text-[11px] text-white/50 font-medium transition-all">{v}</button>
                    ))}
                  </div>
                  ...
                  <div className="space-y-2 px-1">
                    <div className="flex justify-between text-[12px]">
                      ...
                      <span className="text-white/70 font-semibold">
                        {stakeInput && parseFloat(stakeInput) > 0 ? `${(parseFloat(stakeInput) * (100 / (activeTab === 1 ? yesPrice || 1 : (100 - yesPrice) || 1))).toFixed(4)} ETH` : '—'}
                      </span>
                    </div>
                    <div className="flex justify-between text-[12px]">
                    ...
                    <div className="flex justify-between text-[12px]">
                      ...
                      <span className="text-white/50">{pool > 0 ? formatAmount(pool) : '0'}</span>
                    </div>
                  </div>
                  ...
                  <p className="text-[10px] text-white/20 text-center leading-relaxed">
                    ...
                    {(myYesStake > 0 || myNoStake > 0) && <><br />Your stakes: Yes {formatAmount(myYesStake)} · No {formatAmount(myNoStake)}</>}
                  </p>
                </div>
              </div>
            )}
